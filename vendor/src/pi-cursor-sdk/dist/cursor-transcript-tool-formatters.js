...
import { asRecord, getArray, getBoolean, getFirstStringByKeys, getNumber, getRecord, getString, } from "./cursor-record-utils.js";
import { getCursorTaskTranscriptHeader } from "./cursor-task-presentation.js";
import { collectTaskText, getGenerateImageDisplayPath, readMcpDisplayResult, getReadLintDiagnostics, getReadLintPaths, getTodoItems, } from "./cursor-tool-result-display-readers.js";
import { formatDisplayPath, formatDiffString, formatError, formatPathArg, joinSections, limitItems, limitText, LOCAL_READ_PREVIEW_NOTICE, DEFAULT_READ_TRANSCRIPT_CHARS, DEFAULT_READ_TRANSCRIPT_LINES, DEFAULT_NATIVE_READ_DISPLAY_LINES, readFilePreview, stringifyUnknown, truncateArg, } from "./cursor-transcript-utils.js";
export function usesLocalReadPreview(args, result, options) {
...
function getReadContent(args, result, options) {
    const rawPath = typeof args.path === "string" ? args.path : undefined;
    const readOptions = {
        ...options,
        maxChars: options.maxChars ?? DEFAULT_READ_TRANSCRIPT_CHARS,
...
export function formatRead(args, result, options) {
    const rawPath = typeof args.path === "string" ? args.path : undefined;
    const path = rawPath ? formatDisplayPath(rawPath, options.cwd) : "unknown";
    if (result.status === "error")
        return joinSections(`read ${path}`, formatError(result.error));
    const value = asRecord(result.value);
...
export function buildReadDisplayArgs(args, options, result) {
    const rawPath = typeof args.path === "string" ? args.path : undefined;
...
function buildPathDisplayArgs(args, options) {
    const rawPath = typeof args.path === "string" ? args.path : undefined;
    return rawPath ? { ...args, path: formatDisplayPath(rawPath, options.cwd) } : args;
}
export function buildWriteDisplayArgs(args, options) {
...
const CURSOR_EDIT_PATH_KEYS = ["path", "filePath", "file_path"];
const CURSOR_EDIT_OLD_TEXT_KEYS = ["oldText", "old_text", "oldString", "old_string", "oldStr", "old_str"];
const CURSOR_EDIT_NEW_TEXT_KEYS = ["newText", "new_text", "newString", "new_string", "newStr", "new_str"];
const CURSOR_NOTEBOOK_EDIT_ARG_KEYS = ["cellId", "cell_id", "cellIndex", "cell_index", "cellType", "cell_type", "notebookPath", "notebook_path"];
function getCursorEditPathArg(args) {
    const path = getFirstStringByKeys(args, CURSOR_EDIT_PATH_KEYS);
    return path?.trim() ? path : undefined;
}
function isCursorNotebookEditToolName(toolName) {
    const normalized = toolName.replace(/[\s_-]+/g, "").toLowerCase();
    return normalized === "editnotebook" || normalized === "notebookedit";
}
function isCursorStrReplaceToolName(toolName) {
    const normalized = toolName.replace(/[\s_-]+/g, "").toLowerCase();
...
function isCursorNotebookEditActivity(rawToolName, args) {
    if (isCursorNotebookEditToolName(rawToolName))
        return true;
    if (hasAnyKey(args, CURSOR_NOTEBOOK_EDIT_ARG_KEYS))
        return true;
    return !isCursorStrReplaceToolName(rawToolName) && isNotebookPath(getCursorEditPathArg(args));
}
function asNativeEditReplacement(value) {
...
function getNativeEditReplacementsFromArgs(args) {
    const edits = getArray(args, "edits")?.map(asNativeEditReplacement);
    if (edits && edits.length > 0 && edits.every((edit) => edit !== undefined))
        return edits;
    const singleEdit = asNativeEditReplacement(args);
    return singleEdit ? [singleEdit] : undefined;
}
export function buildNativeEditDisplayArgs(rawToolName, args, options) {
    if (isCursorNotebookEditActivity(rawToolName, args))
        return undefined;
...
export function buildCursorEditActivityDisplayArgs(args, options) {
    const rawPath = getCursorEditPathArg(args);
    return rawPath ? { ...args, path: formatDisplayPath(rawPath, options.cwd) } : args;
}
export function formatNativeReadDisplayContent(args, result, options) {
    const value = asRecord(result.value);
    const totalLines = getNumber(value, "totalLines");
    const readOptions = {
        ...options,
...
export function getShellOutput(result, args = {}) {
    const value = asRecord(result.value);
...
export function buildShellDisplayArgs(args) {
    const command = typeof args.command === "string" ? args.command : undefined;
    const timeoutMs = getNumber(args, "timeout");
    const displayArgs = command ? { command } : { ...args };
    if (timeoutMs !== undefined) {
        displayArgs.timeout = timeoutMs / 1000;
    }
    return displayArgs;
}
export function formatShell(args, result, options) {
    ...
    if (executionTime !== undefined)
        outputParts.push(`Took ${(executionTime / 1000).toFixed(1)}s`);
    return joinSections(`$ ${command || "shell"}`, limitText(outputParts.filter(Boolean).join("\n\n"), options));
}
function renderTreeNode(node, depth = 0, lines = []) {
...
export function getLsBody(result, options) {
    const value = asRecord(result.value);
...
export function formatLs(args, result, options) {
    const path = formatPathArg(args, options) ?? ".";
    if (result.status === "error")
        return joinSections(`ls ${path}`, formatError(result.error));
    return joinSections(`ls ${path}`, getLsBody(result, options));
}
export function formatGlob(args, result, options) {
    const header = `$ ${synthesizeGlobBashCommand(args, options)}`;
    if (result.status === "error")
        return joinSections(header, formatError(result.error));
    return joinSections(header, getGlobBody(result, options));
}
function formatSearchCount(totalMatches) {
    return totalMatches === 1 ? "1 match" : `${totalMatches} matches`;
}
function formatSearchFile(file) {
    return file.endsWith(":") ? file.slice(0, -1) : file;
}
function collectSearchResults(value) {
    const record = asRecord(value);
    const outputs = [];
    const activeEditorResult = record?.activeEditorResult;
    if (activeEditorResult)
        outputs.push(activeEditorResult);
    const workspaceResults = asRecord(record?.workspaceResults);
    if (workspaceResults)
        outputs.push(...Object.values(workspaceResults));
    if (outputs.length === 0)
        outputs.push(value);
    const lines = [];
    ...
    for (const outputValue of outputs) {
        ...
        if (type === "content") {
            ...
            if (files.length === 0 && getNumber(output, "totalMatches") === 0)
                sawExplicitNoMatches = true;
            lines.push(...files.filter((entry) => typeof entry === "string").map(formatSearchFile));
        }
        else if (type === "count") {
    ...
    if (lines.length === 0 && sawExplicitNoMatches)
        return ["(no matches)"];
    return lines.filter(Boolean);
}
function synthesizeGrepBashCommand(args, options) {
...
export function buildGrepDisplayArgs(args, options) {
    const displayArgs = {};
    ...
    if (pattern !== undefined)
        displayArgs.pattern = pattern;
    if (path !== undefined)
        displayArgs.path = path;
    if (glob !== undefined)
        displayArgs.glob = glob;
    if (ignoreCase !== undefined)
        displayArgs.ignoreCase = ignoreCase;
    if (context !== undefined)
        displayArgs.context = context;
    if (limit !== undefined)
        displayArgs.limit = limit;
...
function getGlobTargetDirectory(args, options) {
...
function synthesizeGlobBashCommand(args, options) {
...
export function buildFindDisplayArgs(args, options) {
    const displayArgs = { pattern: getGlobPattern(args) };
    const targetDirectory = getGlobTargetDirectory(args, options);
    const limit = getNumber(args, "limit") ?? getNumber(args, "headLimit");
    if (targetDirectory !== undefined)
        displayArgs.path = targetDirectory;
    if (limit !== undefined)
        displayArgs.limit = limit;
    return displayArgs;
}
export function getGrepBody(result, options) {
...
export function getGlobBody(result, options) {
    const value = asRecord(result.value);
    const files = getArray(value, "files")?.filter((entry) => typeof entry === "string") ?? [];
    if (files.length === 0) {
...
export function formatEdit(args, result, options) {
    const path = formatPathArg(args, options) ?? "unknown";
    if (result.status === "error")
        return joinSections(`edit ${path}`, formatError(result.error));
    const value = asRecord(result.value);
    ...
    const body = [stats, diff ? limitText(diff, options) : undefined].filter((part) => Boolean(part)).join("\n\n");
    return joinSections(`edit ${path}`, body || stringifyUnknown(result.value));
}
export function formatDelete(args, result, options) {
    const path = formatPathArg(args, options) ?? "unknown";
    if (result.status === "error")
        return joinSections(`delete ${path}`, formatError(result.error));
    const value = asRecord(result.value);
    const fileSize = getNumber(value, "fileSize");
    return joinSections(`delete ${path}`, fileSize !== undefined ? `Deleted ${fileSize} bytes` : stringifyUnknown(result.value));
}
export function formatReadLints(args, result, options) {
...
export function formatTodos(args, result, options, header) {
    if (result.status === "error")
        return joinSections(header, formatError(result.error));
...
export function formatPlan(args, result, options) {
    if (result.status === "error")
        return joinSections("createPlan", formatError(result.error));
    const planText = getString(args, "plan") ?? getString(asRecord(result.value), "plan");
    if (planText?.trim())
        return joinSections("createPlan", limitText(planText, options));
    return formatTodos(args, result, options, "createPlan");
}
export function formatTask(args, result, options) {
...
export function formatGenerateImage(args, result, options) {
    ...
    const lines = [displayPath ? `Saved image: ${displayPath}` : undefined, hasImageData ? "Image data returned by Cursor SDK." : undefined].filter((line) => Boolean(line));
    if (lines.length > 0)
        return joinSections(`generateImage ${prompt}`, lines.join("\n"));
    return joinSections(`generateImage ${prompt}`, limitText(stringifyUnknown(result.value), options));
}
export function formatSemSearch(args, result, options) {
    ...
    const lines = [];
    if (explanation?.trim())
        lines.push(`Explanation: ${explanation.trim()}`);
    ...
    if (results?.trim())
        lines.push(results.trim());
    const body = lines.length > 0 ? lines.join("\n\n") : stringifyUnknown(result.value);
    return joinSections(header, limitText(body, options));
}
function formatRecordScreenMode(mode) {
    switch (mode) {
...
function formatRecordingDurationMs(ms) {
...
export function formatRecordScreen(args, result, options) {
    ...
    const lines = [];
    if (displayPath)
        lines.push(`Recording: ${displayPath}`);
    if (duration)
        lines.push(`Duration: ${duration}`);
    if (wasCancelled === true)
        lines.push("Prior recording cancelled.");
    if (lines.length === 0) {
        if (mode === "START_RECORDING")
            lines.push("Recording started.");
        else if (mode === "DISCARD_RECORDING")
            lines.push("Recording discarded.");
        else
            lines.push("Screen recording updated.");
    }
    return joinSections(header, lines.join("\n"));
}
function formatWebToolBody(toolLabel, result, options) {
    if (result.status === "error")
        return joinSections(toolLabel, formatError(result.error));
    return joinSections(toolLabel, limitText(readMcpDisplayResult(result).body, options));
}
export function formatWebSearch(args, result, options) {
...
export function formatWebFetch(args, result, options) {
    const target = extractWebFetchTarget(args);
    const header = target ? `web fetch ${target}` : "web fetch";
    return formatWebToolBody(header, result, options);
}
export function formatMcp(args, result, options) {
    const toolName = typeof args.toolName === "string" ? args.toolName : "mcp";
    if (result.status === "error")
        return joinSections(toolName, formatError(result.error));
...
function summarizeUnknownToolArgValue(value) {
    if (typeof value === "string")
        return truncateArg(value);
...
function summarizeUnknownToolArgs(args) {
...
function summarizeUnknownToolResult(value, options) {
...
function summarizeUnknownToolError(error, options) {
...
export function formatFallback(name, args, result, options) {
