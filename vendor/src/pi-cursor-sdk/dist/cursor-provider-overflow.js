 ...
 */
export function normalizeCursorOverflowErrorMessage(errorMessage) {
    const message = errorMessage?.trim();
    if (!message)
        return undefined;
    // Idempotent: never double-prefix an already-normalized message.
    if (message.includes(CURSOR_OVERFLOW_MARKER))
        return undefined;
    if (CURSOR_THROTTLE_PATTERN.test(message))
        return undefined;
 ...
 */
export function rewriteCursorOverflowAssistantMessage(message, isCursorProvider) {
    if (!isCursorProvider || message.stopReason !== "error")
        return undefined;
 ...
 */
export function registerCursorOverflowNormalization(pi) {
