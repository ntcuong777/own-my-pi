...
import { mergeShellOutputDeltasIntoCursorToolCall, } from "./cursor-provider-turn-shell-output.js";
function mergeCursorToolCalls(startedToolCall, completedToolCall) {
...
export function resolveCursorToolCompletion(options) {
    const bridgeStartedCallId = options.ledger.takeBridgeStartedCallId(typeof options.callId === "string" ? options.callId : "");
    ...
    if (options.source === "delta") {
        ...
        resolvedToolCall = mergeShellOutputDeltasIntoCursorToolCall(resolvedToolCall, matchedStartedCallId
        ...
        resolvedToolCall = mergeShellOutputDeltasIntoCursorToolCall(options.toolCall, matchedStartedCallId ? options.shellOutput.takeDeltasForCall(matchedStartedCallId) : undefined);
    ...
    if (options.liveRun?.bridgeRun?.isBridgeMcpToolCall(resolvedToolCall)) {
        const bridgeIdentity = options.source === "step" && matchedStartedCallId ? `cursor-tool:${matchedStartedCallId}` : identity;
        if (bridgeIdentity)
            options.ledger.recordCompletedIdentity(bridgeIdentity);
