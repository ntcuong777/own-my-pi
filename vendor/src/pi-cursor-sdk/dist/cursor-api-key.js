...
export function resolveCursorApiKey(apiKey) {
    const trimmed = apiKey?.trim();
    if (!trimmed)
        return undefined;
    if (CURSOR_API_KEY_PLACEHOLDERS.has(trimmed))
        return process.env.CURSOR_API_KEY?.trim() || undefined;
    return trimmed;
}
function getStoredCursorApiKey() {
    try {
...
export function resolveCursorRuntimeApiKey() {
