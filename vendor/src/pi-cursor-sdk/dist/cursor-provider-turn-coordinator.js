...
import { recordDiscardedIncompleteStartedToolCall, } from "./cursor-sdk-event-debug.js";
import { buildIncompleteCursorToolRunOutcome, resolveIncompleteCursorToolVisibility, } from "./cursor-incomplete-tool-visibility.js";
...
import { createTurnCoordinatorContentEmitter, CursorToolLifecycleEmitter, } from "./cursor-provider-turn-lifecycle-emitter.js";
import { resolveCursorToolCompletion } from "./cursor-provider-turn-sdk-normalizer.js";
import { CursorShellOutputTracker, formatCursorShellOutputProgressText, getCursorShellOutputDelta, isCursorShellToolCall, } from "./cursor-provider-turn-shell-output.js";
import { CursorToolCompletionLedger, getToolFingerprint, } from "./cursor-provider-turn-tool-ledger.js";
import { readCursorSdkTurnUsageFromUpdate } from "./cursor-usage-accounting.js";
export class CursorSdkTurnCoordinator {
    ...
    textDeltas;
    debugRecorder;
    ledger = new CursorToolCompletionLedger();
    shellOutput = new CursorShellOutputTracker();
    displayRouter;
    lifecycleEmitter;
    contentEmitter;
    sdkTurnUsage;
    hasPendingTextMessage = false;
    constructor(options) {
    ...
    get planTextCandidate() {
        return this.displayRouter.planTextCandidate;
    }
    get replayStarted() {
        return this.displayRouter.nativeToolReplayStarted;
    }
    get lastSdkTurnUsage() {
        return this.sdkTurnUsage;
    }
    discardIncompleteStartedToolCalls(outcome = buildIncompleteCursorToolRunOutcome()) {
        for (const [callId, toolCall] of this.ledger.startedToolCallEntries()) {
            ...
            if (visibilityDecision !== "emit") {
                this.displayRouter.recordIncompleteSkip(toolName, visibilityDecision === "debugOnly" && outcome.assistantTextProduced
            ...
            if (action)
                this.displayRouter.emitDisplayAction(action);
        }
    ...
    handleDelta(update) {
        ...
        if (update.type === "tool-call-started") {
            if (this.liveRun?.bridgeRun?.isBridgeMcpToolCall(update.toolCall)) {
                if (typeof update.callId === "string")
                    this.ledger.markBridgeStarted(update.callId);
            }
            else {
                this.lifecycleEmitter.maybeSchedule(update.callId, update.toolCall);
    ...
    handleStep(stepEnvelope) {
        ...
        if (stepType === "assistantMessage") {
            ...
            if (this.hasPendingTextMessage) {
                ...
                if (this.liveRun)
                    cursorLiveRuns.queueEvent(this.liveRun, { type: "text-completed" });
                else
                    this.contentEmitter.completeTextMessage();
            }
            return;
        }
