...
export function isCursorToolLifecycleEligible(toolCall) {
    return classifyCursorToolVisibility(toolCall).lifecycleEligible;
}
function getCursorToolLifecycleTitle(visibility) {
...
function containsCursorLifecycleUnsafeDetail(text) {
    if (/\b[a-z][a-z0-9+.-]*:\/\//i.test(text))
        return true;
    if (/\bwww\.\S+/i.test(text))
        return true;
    if (new RegExp(`${LIFECYCLE_DETAIL_PATH_PREFIX}~\\/\\S*`).test(text))
        return true;
    if (new RegExp(`${LIFECYCLE_DETAIL_PATH_PREFIX}\\/\\S+`).test(text))
        return true;
    if (new RegExp(`${LIFECYCLE_DETAIL_PATH_PREFIX}[A-Za-z]:[\\\\/]`).test(text))
        return true;
    return false;
}
function scrubLifecycleDetail(value, apiKey) {
    if (!value?.trim())
        return undefined;
    const scrubbed = truncateCursorDisplayLine(scrubSensitiveText(value, apiKey));
    if (containsCursorLifecycleUnsafeDetail(scrubbed))
        return undefined;
    return scrubbed;
}
function scrubShellLifecycleDetail(value, apiKey) {
    if (!value?.trim())
        return undefined;
    return truncateCursorDisplayLine(scrubSensitiveText(value, apiKey));
}
function buildCursorToolLifecycleLabelFromVisibility(visibility, apiKey) {
    ...
    switch (getCursorToolLifecycleLabelKind(visibility.normalizedKey)) {
        ...
        case "updateTodos": {
            ...
            if (todos && todos.length > 0)
                return truncateArg(`${todos.length} item${todos.length === 1 ? "" : "s"}`);
            return "todos";
        }
        default:
            return undefined;
...
export function buildCursorToolLifecycleLabel(toolCall, apiKey) {
    return buildCursorToolLifecycleLabelFromVisibility(classifyCursorToolVisibility(toolCall), apiKey);
}
export function formatCursorToolLifecycleProgressText(toolCall, apiKey) {
