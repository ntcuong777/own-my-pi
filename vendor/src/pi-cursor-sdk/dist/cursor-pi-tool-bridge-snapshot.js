...
export { CURSOR_PI_TOOL_BRIDGE_BUILTINS_ENV, CURSOR_PI_TOOL_BRIDGE_ENV, resolveCursorPiToolBridgeBuiltinsEnabled, resolveCursorPiToolBridgeEnabled, } from "./cursor-pi-tool-bridge-env.js";
...
function isOverlappingCursorNativePiToolName(toolName) {
    return OVERLAPPING_CURSOR_NATIVE_PI_BUILTIN_TOOL_NAMES.has(toolName);
}
export function buildCursorPiToolBridgeSurfaceSignature(snapshot) {
...
export function buildCursorPiToolBridgeSnapshot(pi, options = {}) {
    const activeToolNames = new Set(pi.getActiveTools());
    const allTools = pi.getAllTools();
    const usedMcpToolNames = new Set();
    const mcpToolNameToPiToolName = new Map();
    const piToolNameToMcpToolName = new Map();
    const tools = [];
    const exposeOverlappingBuiltins = options.exposeOverlappingBuiltins === true;
    for (const tool of allTools) {
        if (!activeToolNames.has(tool.name))
            continue;
        if (isExcludedFromCursorBridgeExposure(tool.name) && isRegisteredCursorNativeToolName(tool.name))
            continue;
        if (!exposeOverlappingBuiltins && isOverlappingCursorNativePiToolName(tool.name))
            continue;
