...
import { DISCARDED_INCOMPLETE_TOOL_CALL_REASON, } from "./cursor-sdk-event-debug.js";
import { assembleCursorReplayActivityDetails, parseCursorReplayToolDetails, resolveIncompleteReplayActivitySourceToolName, } from "./cursor-replay-tool-details.js";
import { asRecord } from "./cursor-record-utils.js";
import { classifyCursorToolVisibility } from "./cursor-tool-visibility.js";
export function buildIncompleteCursorToolRunOutcome(outcome = {}) {
    return {
...
export function resolveIncompleteCursorToolVisibility(toolCall, outcome) {
    const visibility = classifyCursorToolVisibility(toolCall);
    if (outcome.reason === DISCARDED_INCOMPLETE_TOOL_CALL_REASON &&
...
export function formatIncompleteCursorToolReasonText(reason) {
    switch (reason) {
...
export function getIncompleteCursorToolActivityTitle(toolCall) {
    const visibility = classifyCursorToolVisibility(toolCall);
    return visibility.incompleteTitle ?? getCursorToolActivityTitle(visibility.displayName);
}
export function buildIncompleteCursorToolDisplay(toolCall, reason, options = {}) {
    const visibility = classifyCursorToolVisibility(toolCall);
    ...
    const details = assembleCursorReplayActivityDetails(resolveIncompleteReplayActivitySourceToolName(visibility.normalizedName), headline, { summary: reasonText, expandedText: contentText }, contentText, true, reasonText);
    return {
...
export function formatIncompleteCursorToolTrace(display) {
    ...
    const title = (typeof detailRecord?.title === "string" && detailRecord.title.trim()) ||
    ...
    const summary = (typeof detailRecord?.summary === "string" && detailRecord.summary.trim()) ||
