...
export async function cacheSdkContextWindow(agentId, modelId, cwd, store) {
    try {
        const { createAgentPlatform } = await loadCursorSdk();
        const platform = await createAgentPlatform(cwd || store
            ? {
        ...
        if (contextWindow)
            saveCachedContextWindow(modelId, contextWindow);
    }
    catch {
        // Context-window cache failures must not affect response streaming.
    }
}
export function buildCursorRunOutcomeFromWait(params) {
