...
import { fingerprintApiKey, loadAnyCachedModelCatalog, loadFreshCachedModels, saveModelListCache, } from "./model-list-cache.js";
...
const CATALOG_REFRESH_HINT = "After adding auth to an already-started pi session, run /cursor-refresh-models to refresh the full live Cursor model catalog without restarting pi.";
async function getDiscoveryApiKey(apiKey) {
...
function getParameterValue(parameter, lowerValue) {
    const value = parameter?.values.find((candidate) => candidate.value.toLowerCase() === lowerValue);
    return value?.value ?? null;
}
function getPreferredParameterValue(parameter, lowerValues) {
...
function getThinkingLevelMap(item) {
    ...
    return {
        off: getParameterValue(reasoningParameter, "none") ??
...
function toModelConfig(metadata, name) {
    return {
...
export function getCursorModelMetadataEntries() {
...
function deleteParam(params, id) {
...
export function buildCursorModelSelection(modelId, thinkingLevel, fastEnabled) {
...
async function useFallbackModels(options, issue) {
