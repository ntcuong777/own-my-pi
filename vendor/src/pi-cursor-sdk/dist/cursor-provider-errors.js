...
function isGenericCursorRunFailureMessage(message) {
    return /^cursor sdk run failed\.?$/i.test(message.trim());
}
function isKnownGenericRunFailureText(message) {
    const normalized = message.trim().toLowerCase();
...
function getErrorStringField(record, key) {
...
const CONNECT_ERROR_EVIDENCE_KEYS = ["name", "message", "rawMessage", "code", "syscall"];
function getErrorEvidenceField(value, record, key) {
    if (value instanceof Error && (key === "name" || key === "message"))
...
function collectConnectErrorEvidence(error, record) {
    const evidence = [];
    let value = error;
...
function collectConnectErrorStacks(error, record) {
    const stacks = [];
    let value = error;
    let currentRecord = record;
    for (let depth = 0; depth < 3 && currentRecord; depth += 1) {
...
function isConnectError(error, record) {
    const name = error instanceof Error ? error.name : getErrorStringField(record, "name");
    return name === "ConnectError";
}
function isUnauthenticatedConnectCode(code) {
    return code === 16 || (typeof code === "string" && /^(?:16|unauthenticated)$/i.test(code));
}
function isUnavailableConnectCode(code) {
    return code === 14 || (typeof code === "string" && /^(?:14|unavailable)$/i.test(code));
}
function isCursorSdkStallAbortNetworkError(code, evidence, stackEvidence) {
    return ((code === 2 || (typeof code === "string" && /^(?:2|unknown)$/i.test(code))) &&
 ...
 */
export function isCursorSdkConnectionStalledError(error) {
...
function getCursorConnectSource(error, record) {
    const stack = getErrorStack(error, record);
    if (stack.includes("@cursor/sdk"))
        return "cursor-sdk-stack";
    if (isCursorExtensionConnectStack(stack))
        return "cursor-extension-connect-stack";
    ...
    if (hasCursorBackendDetails)
        return "cursor-backend-details";
    return stack.includes("@connectrpc/connect-node") ? "connect-node-stack" : "generic-connect";
}
export function classifyCursorConnectError(error) {
    const record = asRecord(error);
...
export function isCursorSdkAbortConnectError(error) {
...
function runErrorCode(error) {
    return error?.code?.trim() || undefined;
}
function nonGenericRunErrorMessage(error) {
    const message = error?.message?.trim();
    return message && !isKnownGenericRunFailureText(message) ? message : undefined;
}
function withRunErrorCode(message, code) {
    return code ? `${message} (code: ${code})` : message;
}
export function formatCursorSdkRunFailureDetail(result, runResult, runError) {
    ...
    if (fromWaitError)
        return withRunErrorCode(fromWaitError, errorCode);
    ...
    if (fromRunError)
        return withRunErrorCode(fromRunError, errorCode);
    ...
    if (result.model?.id)
        parts.push(`model ${result.model.id}`);
    parts.push(`run ${shortRunId(result.id)}`);
    if (result.requestId)
        parts.push(`request ${shortRunId(result.requestId)}`);
    if (typeof result.durationMs === "number")
        parts.push(`${result.durationMs}ms`);
    return parts.join(" · ");
}
export function formatCursorSdkAbortMessage(cause) {
    switch (cause) {
        case "user_interrupt":
...
export function resolveCursorSdkAbortCause(options) {
    ...
    if (options.sdkStatusCancelled)
        return "sdk_cancelled";
    if (options.liveRunDisposed)
...
export function sanitizeCursorProviderError(error, apiKey, runtimeTarget) {
    ...
    if (isGenericCursorRunFailureMessage(scrubbed))
        return RETRYABLE_CURSOR_RUN_FAILURE_PREFIX;
    if (isGenericErrorMessage(scrubbed))
        return GENERIC_CURSOR_SDK_ERROR_MESSAGE;
