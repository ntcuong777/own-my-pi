import { CURSOR_APPROX_CHARS_PER_TOKEN, CURSOR_IMAGE_TOKEN_ESTIMATE, estimateCursorContextTokens, estimateCursorTextTokens, } from "./context.js";
import { asRecord, getNumber } from "./cursor-record-utils.js";
function getPromptInputTokenBudget(model) {
...
export function getCursorPromptOptions(model) {
    return {
...
function getNonNegativeTokenCount(record, key) {
...
export function readCursorSdkTurnUsageFromUpdate(update) {
    const record = asRecord(update);
    return record?.type === "turn-ended" ? readCursorSdkTurnUsage(record.usage) : undefined;
}
function stringifyUsageValue(value) {
    try {
...
export function estimateCursorAssistantSessionOutputTokens(message) {
...
function withAssistantMessage(context, partial) {
    return { ...context, messages: [...context.messages, partial] };
}
export function estimateCursorContextTotalTokens(partial, model, context) {
    return estimateCursorContextTokens(withAssistantMessage(context, partial), getCursorPromptOptions(model));
}
function getCursorSdkUncachedInputTokens(turnUsage) {
...
export function isCursorSdkUsagePartitionSafe(turnUsage, model) {
    ...
    return (counts.every((count) => Number.isFinite(count) && count >= 0) &&
...
function isCompatibleCursorAssistantMeasurement(assistant, model) {
    return assistant.api === model.api && assistant.provider === model.provider && assistant.model === model.id;
}
function getLatestCompactionBoundary(context) {
...
function getLastAcceptedContextOccupancy(context, model) {
    ...
    for (let index = context.messages.length - 1; index >= 0; index -= 1) {
        ...
        if (assistant.stopReason === "aborted" || assistant.stopReason === "error" || !assistant.usage)
            continue;
        if (!isCompatibleCursorAssistantMeasurement(assistant, model))
            continue;
        const { usage } = assistant;
        const total = usage.totalTokens || usage.input + usage.output + usage.cacheRead + usage.cacheWrite;
        if (!Number.isFinite(total) || total <= 0 || total > model.contextWindow)
            continue;
        if (boundary?.tokensBefore !== undefined && total >= boundary.tokensBefore)
            continue;
        return total;
    }
    return 0;
}
export function applyCursorApproximateUsage(partial, model, context, sessionInputTokens) {
    ...
    partial.usage.totalTokens = Math.max(partial.usage.input + partial.usage.output, estimateCursorContextTotalTokens(partial, model, context), getLastAcceptedContextOccupancy(context, model));
}
function applyCursorOccupancyEstimate(partial, model, context) {
    partial.usage.totalTokens = Math.max(estimateCursorContextTotalTokens(partial, model, context), getLastAcceptedContextOccupancy(context, model));
}
function isCurrentLocalOccupancy(turn, model, context) {
    if (!isCursorSdkUsageSafeForPiMessage(turn, model))
        return false;
...
function applyResolvedCursorOccupancy(partial, model, context, localTurn) {
