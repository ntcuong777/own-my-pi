import { CURSOR_KNOWN_NORMALIZED_TOOL_NAMES, CURSOR_REPLAY_ACTIVITY_TOOL_NAME, getCursorReplayCallSummary, getCursorToolActivityTitle, getCursorToolActivityReplaySpec, getCursorToolGenerateImageReplaySpec, } from "./cursor-tool-presentation-registry.js";
import { resolveCursorEditDiff } from "./cursor-edit-diff.js";
import { assembleCursorReplayActivityDetails, assembleCursorReplayGenerateImageDetails, CURSOR_REPLAY_UNREGISTERED_ACTIVITY_TOOL_NAME, } from "./cursor-replay-tool-details.js";
import { asRecord, getNumber, getString } from "./cursor-record-utils.js";
import { firstNonEmptyLine, formatDisplayPath, formatDiffString, formatError, limitText, truncateArg, } from "./cursor-transcript-utils.js";
import { buildCursorEditActivityDisplayArgs, buildFindDisplayArgs, buildGrepDisplayArgs, buildNativeEditDisplayArgs, buildReadDisplayArgs, buildShellDisplayArgs, buildWriteDisplayArgs, formatDelete, formatEdit, formatFallback, formatGenerateImage, formatGlob, formatGrep, formatLs, formatMcp, formatWebFetch, formatWebSearch, formatPlan, formatRecordScreen, formatSemSearch, formatRead, formatReadLints, formatShell, formatTask, formatTodos, formatWrite, formatNativeReadDisplayContent, getCursorWriteArgContent, getGlobBody, getGrepBody, getLsBody, getShellOutput, usesLocalReadPreview, } from "./cursor-transcript-tool-formatters.js";
function textToolResult(text, details) {
    return { content: [{ type: "text", text }], details };
}
function buildCursorActivityDisplayArgs(args, activityTitle, activitySummary) {
...
function buildReplaySummaryDisplay(toolName, args, result, contentText, details) {
...
function buildActivityReplayDisplay(sourceToolName, context) {
    const spec = TOOL_DISPLAY_IMPLEMENTATIONS[sourceToolName];
    const activity = getCursorToolActivityReplaySpec(sourceToolName);
    if (!activity)
        throw new Error(`Missing activity replay spec for ${sourceToolName}`);
    ...
    const details = assembleCursorReplayActivityDetails(sourceToolName, activityTitle, activityFields, contentText, context.result.status === "error", activitySummary);
    return buildReplaySummaryDisplay(CURSOR_REPLAY_ACTIVITY_TOOL_NAME, activityArgs, context.result, contentText, details);
}
function buildGenerateImageReplayDisplay(context) {
    const spec = TOOL_DISPLAY_IMPLEMENTATIONS.generateImage;
    const replay = getCursorToolGenerateImageReplaySpec("generateImage");
    if (!replay)
        throw new Error("Missing generate image replay spec");
    ...
    const activityArgs = buildCursorActivityDisplayArgs({ ...replayArgs }, activityTitle, activitySummary);
    const contentText = spec.formatTranscript(context).trimEnd();
    const details = assembleCursorReplayGenerateImageDetails(replay.buildDetails(context, contentText), contentText, context.result.status === "error", activitySummary);
    return buildReplaySummaryDisplay(CURSOR_REPLAY_ACTIVITY_TOOL_NAME, activityArgs, context.result, contentText, details);
}
function buildGenericPiToolDisplay(context) {
    ...
    const activitySummary = result.status === "error" ? undefined : firstNonEmptyLine(fallbackBody);
    ...
    return buildReplaySummaryDisplay(CURSOR_REPLAY_ACTIVITY_TOOL_NAME, activityArgs, result, contentText, assembleCursorReplayActivityDetails(CURSOR_REPLAY_UNREGISTERED_ACTIVITY_TOOL_NAME, activityTitle, { summary, expandedText: contentText }, contentText, result.status === "error", activitySummary));
}
function buildEditActivitySummary(displayPath, value) {
...
function buildEditPiToolDisplay(context) {
    ...
    return buildReplaySummaryDisplay(CURSOR_REPLAY_ACTIVITY_TOOL_NAME, activityArgs, result, contentText.trimEnd(), assembleCursorReplayActivityDetails("edit", activityTitle, {
        path: displayPath,
        ...
        linesAdded: getNumber(value, "linesAdded"),
        linesRemoved: getNumber(value, "linesRemoved"),
    }, contentText.trimEnd(), result.status === "error", displayPath));
}
function buildWritePiToolDisplay(context) {
    ...
    if (content === undefined) {
        ...
        return buildReplaySummaryDisplay(CURSOR_REPLAY_ACTIVITY_TOOL_NAME, buildCursorActivityDisplayArgs(displayArgs, activityTitle, displayPath ?? "file"), result, contentText, assembleCursorReplayActivityDetails("write", activityTitle, {
            path: displayPath,
            summary: result.status === "error" ? undefined : displayPath ?? "wrote file",
            expandedText: contentText,
            fileContentAfterWrite: getString(value, "fileContentAfterWrite"),
        }, contentText, result.status === "error", displayPath ?? "file"));
    }
    return {
        toolName: "write",
        args: displayArgs,
...
const TOOL_DISPLAY_IMPLEMENTATIONS = {
    read: {
        ...
        buildPiToolDisplay: ({ args, result, options }) => {
            ...
            return {
                ...
                result: textToolResult(isError ? formatError(result.error) : formatNativeReadDisplayContent(args, result, options), usesLocalPreview ? { localReadPreview: true } : undefined),
                isError,
            };
        },
    },
...
function getToolDisplaySpec(name) {
    if (Object.hasOwn(TOOL_DISPLAY_IMPLEMENTATIONS, name))
        return TOOL_DISPLAY_IMPLEMENTATIONS[name];
    return undefined;
}
export function formatCursorToolTranscriptFromSpec(context) {
    const spec = getToolDisplaySpec(context.name);
    if (spec)
        return spec.formatTranscript(context);
    return formatFallback(context.name, context.args, context.result, context.options);
}
export function buildCursorPiToolDisplayFromSpec(context) {
    const spec = getToolDisplaySpec(context.name);
    if (spec)
        return spec.buildPiToolDisplay(context);
