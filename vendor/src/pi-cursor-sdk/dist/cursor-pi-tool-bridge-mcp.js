...
export function normalizeMcpArgs(args) {
    const record = asRecord(args);
    return record ? { ...record } : {};
}
export function waitForProtocolFlush() {
    return new Promise((resolve) => setTimeout(resolve, 0));
}
function sanitizeMcpToolNameStem(toolName) {
    const stem = toolName
        .trim()
...
export function stableNameHash(value) {
    return createHash("sha256").update(value).digest("hex").slice(0, 8);
}
export function createMcpToolName(piToolName, usedMcpToolNames) {
...
export function snapshotToolToMcpTool(tool) {
    return {
...
export function convertPiContentToMcpContent(content) {
...
export function asToolResultMessage(value) {
    return value.role === "toolResult" ? value : undefined;
}
export function containsKnownMcpToolName(value, knownMcpToolNames, depth = 0) {
    if (depth > 4)
        return false;
    ...
    for (const field of ["tool", "toolName", "name", "mcpToolName", "serverToolName"]) {
        ...
        if (typeof fieldValue === "string" && knownMcpToolNames.has(fieldValue))
            return true;
    }
    for (const nestedField of ["args", "arguments", "input"]) {
        if (containsKnownMcpToolName(record[nestedField], knownMcpToolNames, depth + 1))
            return true;
    }
    return false;
}
