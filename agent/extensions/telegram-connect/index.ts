...
function topicLabel(config: Partial<TelegramConnectConfig>): string {
	return config.topicName ?? (config.topicId ? `topic #${config.topicId}` : "main chat");
}
...
export default function telegramConnectExtension(pi: ExtensionAPI) {
	...
	function updateStatus(ctx: ExtensionContext): void {
		...
		setStatus("telegram", theme.fg("accent", label));
	}

	// ── Telegram API helpers (capture token/chatId from config) ────────

	/** Convenience accessors — only call when isActive(config) is true. */
	function tgSendMessage(text: string, options?: tg.SendMessageOptions) {
		return tg.sendMessage(config.botToken, config.chatId, text, options);
	}
	function tgReact(messageId: number, emoji: string) {
		if (useBridge) return reactViaBridge("telegram", messageId, emoji);
		return tg.setMessageReaction(config.botToken, config.chatId, messageId, emoji);
	}
	function tgAnswerCallback(callbackId: string) {
		if (useBridge) return answerCallbackViaBridge("telegram", callbackId);
		return tg.answerCallbackQuery(config.botToken, callbackId);
	}
	function tgEditMarkup(messageId: number) {
		if (useBridge) return editMarkupViaBridge("telegram", messageId);
		return tg.editMessageReplyMarkup(config.botToken, config.chatId, messageId);
	}
	function tgTyping() {
		return tg.sendChatAction(config.botToken!, config.chatId!, "typing", config.topicId);
	}
	function tgDraft(draftId: number, text: string) {
		return tg.sendMessageDraft(config.botToken, config.chatId, draftId, text, { messageThreadId: config.topicId });
	}
	...
	async function send(text: string, extraOptions?: Partial<tg.SendMessageOptions>): Promise<number | null> {
		...
		// ── Bridge mode: route outgoing through bridge ──────────────────
		if (useBridge) {
			const replyTo = (restExtra as any).reply_parameters?.message_id ?? 0;
			const result = await sendViaBridge("telegram", config.topicId ?? 0, formatted, {
				parseMode: "HTML",
				replyTo,
				replyMarkup: reply_markup,
			});
			if (result.ok) return result.messageId ?? null;

			// Bridge send failed — fall through to direct send as fallback
		}

		// ── Direct mode: send via Telegram API ──────────────────────────
		...
		for (let i = 0; i < chunks.length; i++) {
			...
			try {
				const msg = await tgSendMessage(chunks[i]!, {
					...topic,
					parse_mode: "HTML",
					...restExtra,
					...markupOpt,
				...
				for (let j = 0; j < plainChunks.length; j++) {
					...
					const msg = await tgSendMessage(plainChunks[j]!, {
						...topic,
						...restExtra,
						...plainMarkup,
					}).catch(() => null);
	...
	async function sendPlain(text: string): Promise<void> {
		...
		if (useBridge) {
			await sendViaBridge("telegram", config.topicId ?? 0, text);
			return;
		}
		const chunks = chunkText(text);
		for (const chunk of chunks) {
			await tgSendMessage(chunk, topicOptions(config)).catch(() => {});
		}
	}
	...
	async function flushDraft(flush: DraftFlush): Promise<FlushResult> {
		...
		try {
			await tgDraft(flush.draftId, flush.text);
			return "ok";
		} catch (err) {
	...
	/** Try bridge first, fall back to direct polling. Returns mode label. */
	async function startConnection(ctx: ExtensionContext): Promise<"bridge" | "direct"> {
		if (!isConfigured(config)) return "direct";
		const bridgeOk = await connectViaBridge(ctx);
		if (bridgeOk) return "bridge";
		if (!pollingAbort) {
			const offset = await tg.getNextUpdateOffset(config.botToken).catch(() => 0);
			startPolling(offset).catch(() => {});
		}
		return "direct";
	}
	...
	function handleBridgeEvent(evt: BridgeEvent): void {
		...
		if (evt.type === "callback" && evt.callbackId) {
			handleCallbackFromBridge(evt);
			return;
		}
	...
	async function handleSetupPairing(msg: tg.TelegramMessage): Promise<void> {
		...
		await registerBotCommands(config.botToken!, config.chatId);
		await tgSendMessage("✅ Paired and connected! This session is now linked. Future sessions need /telegram or /telegram start to reconnect.", topicOptions(config)).catch(() => {});
	}
	...
	async function handleCallbackQuery(cq: tg.TelegramCallbackQuery): Promise<void> {
		...
		if (cq.data.startsWith("tc:")) {
			...
			if (interaction.messageId !== null) {
				await tgEditMarkup(interaction.messageId).catch(() => {});
			}
			if (isStreaming) {
	...
	/** Handle callback query received via bridge SSE. */
	function handleCallbackFromBridge(evt: BridgeEvent): void {
		if (!isConfigured(config) || !evt.text) return;

		// Answer the callback to dismiss the spinner on Telegram
		if (evt.callbackId) {
			tgAnswerCallback(evt.callbackId).catch(() => {});
		}

		if (evt.text.startsWith("tc:")) {
			...
			pendingInteractions.delete(interactionId);
			if (interaction.messageId !== null) {
				tgEditMarkup(interaction.messageId).catch(() => {});
			}
			if (isStreaming) {
				messageQueue.push({ text: opt.submit_text, messageId: evt.messageId ?? 0 });
			} else {
				runTriggerMessageId = evt.messageId ?? null;
				pi.sendUserMessage(opt.submit_text);
			}
		} else {
			pi.events.emit("telegram:callback", { data: evt.text });
		}
	}
	...
	async function handleCommand(text: string): Promise<void> {
		...
		switch (cmd) {
			case "/abort":
				cancelPendingText();
				await tgSendMessage("⛔ Abort signal sent.", topicOptions(config)).catch(() => {});
				if (isStreaming) {
			...
			case "/status": {
				...
				await tgSendMessage(html, { ...topicOptions(config), parse_mode: "HTML" }).catch(() => {});
				break;
			}
	...
	async function handlePhoto(msg: tg.TelegramMessage): Promise<void> {
		await tgReact(msg.message_id, "👀").catch(() => {});
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		if (isFresh) {
			await sendPlain(`🟢 Pi started · ${topicLabel(config)}`).catch(() => {});
		}
	});
	...
	pi.on("session_switch", async (_event, ctx) => {
		...
		await reloadConfigForCwd(ctx);
		if (!isActive(config)) return;
		await sendPlain(`📂 Session switched · ${topicLabel(config)}`).catch(() => {});
	});

	// NOTE: The buttons-capability hint must be delivered via `systemPrompt`,
	// NOT via `message`. Returning `{ message: { content: ... } }` here injects
	// a *persistent user-shaped message* into the session on every agent run,
	// which makes the LLM see the hint as if the user had typed it each turn
	// (and it also gets relayed back through user-content code paths). The
	// idiomatic delivery for a capability hint is to append it to the chained
	// system prompt — pi recomputes this per turn but it never enters the
	// session message log.
	...
	// message_end handler: strips the telegram-buttons JSON trailer
	// from the local TUI rendering by returning a replacement message.
	// The Telegram-side parser at the agent_end handler below is
	// unaffected — it consumes the raw assistant text via
	// parseChoicesBlock at the same call site it always has, before
	// this handler's replacement is observed there. Streaming pre-render
	// of the raw text briefly shows the JSON; the snap-to-clean happens
	// at message_end. This was a deliberate tradeoff (option (b)) chosen
	// over a full tool-call refactor (option (c)) for the smaller
	// footprint. See conversation history if revisiting.
	//
	// Per pi 0.74.0 docs (extensions.md, message_end): return
	// `{ message }` to replace the finalized message; replacement must
	// keep the same `role`. Do not mutate `event.message` — pi's core
	// persists the original object to JSONL otherwise.
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	(pi as any).on("message_end", async (event: any) => {
		...
		// Find the last text content block. The system prompt instructs
		// the model to emit the JSON block as the very last thing, so it
		// will live inside the trailing text block.
		let lastTextIdx = -1;
		...
		const lastText = msg.content[lastTextIdx] as { type: "text"; text: string };
		const { visibleText, choices } = parseChoicesBlock(lastText.text);
		// Trust parseChoicesBlock: it returns choices === null for any
		// non-match or malformed JSON, in which case we render unchanged
		// so the bug stays visible.
		if (!choices) return;
		if (visibleText === lastText.text) return;

		const newContent = msg.content.slice();
		if (visibleText.length === 0) {
			newContent.splice(lastTextIdx, 1);
			if (newContent.length === 0) {
				// Avoid leaving an empty-content assistant message; fall back
				// to a single empty text block to preserve message shape.
				newContent.push({ type: "text", text: "" });
			}
		} else {
			newContent[lastTextIdx] = { ...lastText, text: visibleText };
		}
	...
	pi.on("tool_call", async (event) => {
		...
		await tgDraft(draftState.draftId, displayText).catch(() => {});
	});
	...
	pi.on("agent_end", async (event, ctx) => {
		...
		if (isActive(config) && runTriggerMessageId !== null) {
			await tgReact(runTriggerMessageId, "✅").catch(() => {});
		}
	...
	pi.events.on("permission-gate:waiting", (payload: any) => {
		if (!isActive(config)) return;
		const cmd = payload?.command ?? "unknown command";
		const labels = payload?.labels ?? "";
		// Send with inline yes/no buttons
		const text = `⚠️ <b>Permission gate</b> — ${escapeHTML(topicLabel(config))}\n\n<code>${escapeHTML(cmd)}</code>\n\n<i>${escapeHTML(labels)}</i>`;
		const keyboard: tg.InlineKeyboardMarkup = {
			inline_keyboard: [[
				{ text: "✅ Allow", callback_data: "gate:allow" },
				{ text: "❌ Block", callback_data: "gate:block" },
			]],
		};
		void tgSendMessage(text, {
			parse_mode: "HTML",
	...
	// Handle gate approval/rejection from Telegram buttons
	pi.events.on("telegram:callback", (payload: any) => {
		if (payload?.data === "gate:allow") {
			pi.events.emit("permission-gate:respond", { allow: true });
		} else if (payload?.data === "gate:block") {
			pi.events.emit("permission-gate:respond", { allow: false, reason: "Blocked from Telegram" });
		}
	});
		...
		handler: async (args, ctx) => {
			...
			if (!sub) {
				...
				if (config.enabled && isConfigured(config)) {
					const mode = await startConnection(ctx);
					ctx.ui.notify(`Telegram enabled (${mode}) — ${topicLabel(config)}`, "info");
			...
			switch (sub) {
				...
				case "start": {
					...
					if (isConfigured(config)) {
						const mode = await startConnection(ctx);
						ctx.ui.notify(`Telegram started (${mode}) — ${topicLabel(config)}`, "info");
					}
					break;
				}
				...
				case "topic": {
					...
					// If topic already exists for this project, confirm before replacing
					if (config.topicId) {
					...
					// Name from args, or prompt, or fall back to directory name
					let topicName = subArgs.trim();
					if (!topicName) {
						const input = await ctx.ui.input(
							"Topic name",
							`Name for the forum topic (leave blank for "${basename(ctx.cwd)}")`,
						);
						if (input === null) { ctx.ui.notify("Cancelled", "info"); break; }
						topicName = input.trim() || basename(ctx.cwd);
					}
					...
					try {
						...
						ctx.ui.notify(
							`✅ Topic created: "${topicName}" (#${message_thread_id})\nAll messages now routed here.`,
							"info",
						);
					} catch (err) {
						ctx.ui.notify(
