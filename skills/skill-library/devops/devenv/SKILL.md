...
devenv works best with direnv for automatic environment activation when you `cd` into
# the project. See the [.envrc template](./assets/templates/dot_envrc) for the recommended
# configuration:

```bash
# Create .envrc from template (copy from assets/templates/dot_envrc)
cat > .envrc << 'EOF'
#!/usr/bin/env bash
...
# Allow direnv to evaluate .envrc (do this once per project)
direnv allow
```

After `direnv allow`, the devenv environment activates automatically whenever
...

{
  ...
  # For secrets, use dotenv.enable = true + gitignored .env
  env.DATABASE_URL = "postgresql:///mydb?host=$PGHOST";

  # Load secrets from .env (keep .env in .gitignore!)
  dotenv.enable = true;
...
- **[dot_envrc](./assets/templates/dot_envrc)** - direnv integration for automatic environment activation
...
### 7. Setup direnv Integration

**User wants:** Automatic environment activation when entering project directory

**Steps:**
1. Ensure direnv is installed: `nix profile install nixpkgs#direnv`
2. Use the **[dot_envrc template](./assets/templates/dot_envrc)**
3. Copy to project root as `.envrc`
4. Run `direnv allow` once per project
5. The devenv environment now auto-activates on `cd`

### Security Notes

- **Never put secrets (API keys, passwords, tokens) in `env.*`** — `devenv.nix` is source-controlled and values in `env.*` are visible to anyone with repo access
- Use `dotenv.enable = true` and a gitignored `.env` file for secrets
- For per-developer environment variables, use direnv's `.envrc` (gitignore it if it contains secrets)
- Prefer `dotenv.enable` over direct `env.*` for sensitive values
...
When generating or modifying `devenv.nix`, always validate the configuration to catch errors before the user sees them. Agent training data may be outdated — devenv's option names, types, and module structure evolve over time. Never assume your knowledge is current; verify with the commands below.

### Validation Commands

| Command | Purpose |
|---------|---------|
| `devenv shell -- true` | Evaluate + build config, then exit immediately (no-op check) |
| `devenv info` | Print resolved configuration — verify options took effect |
| `devenv shell -vvv` | Verbose evaluation — shows full Nix trace on errors |
| `nix flake check` | If using flakes, validates flake outputs |
| `devenv build` | Build the environment derivation without entering |

### Workflow: Validate After Every Change

1. **Write the config** — generate or modify `devenv.nix`
2. **Validate immediately** — run `devenv shell -- true` to check for evaluation errors
3. **Parse the error** — read the error message carefully:
   - `error: attribute 'X' missing` → option was renamed or never existed
   - `error: value is X but Y was expected` → type mismatch (e.g., string vs int)
   - `error: The option 'X' does not exist` → deprecated or incorrect path
   - `error: A definition for option 'X' is not of type 'Y'` → wrong type passed
4. **Find the correct option** — use `search_options` MCP tool to query live devenv docs:
   ```
   search_options("python virtualenv")  # finds current Python-related options
   search_options("postgres port")      # finds current PostgreSQL options
   ```
5. **Fix and re-validate** — apply the correction and run step 2 again

### Common Validation Errors & Likely Causes

| Error pattern | Likely cause | Fix |
|---|---|---|
| `attribute 'python' missing` under `languages` | Python option structure changed | Run `search_options("python")` and check current nesting |
| `enumOf` or type mismatch | Option type changed (bool → enum, string → path, etc.) | Check `search_options` for accepted values |
| `The option ... does not exist` | Option was removed or renamed in newer devenv | Search for the concept, not the exact old name |
| Build/service startup failure | Service defaults or required fields changed | Check `search_options` for new required fields |
| `error: undefined variable` | Package moved between nixpkgs or renamed | Use `search_packages` to find the current package name |
| `infinite recursion encountered` | Self-referential config (e.g. a process depending on itself) | Check process dependency graph |

### Fast Feedback Loop

For rapid iteration, keep a one-liner check handy:

```bash
# Quick pass/fail check
devenv shell -- true && echo "✓ Config OK" || echo "✗ Config has errors"

# Watch mode (re-check on file save, Linux/macOS)
while inotifywait -e close_write devenv.nix 2>/dev/null; do
  echo "=== Checking config ==="
  devenv shell -- true 2>&1 | tail -20
  echo "=== Done ==="
done
```

### When Agent Knowledge May Be Stale

If a generated config produces errors:

1. **Don't** assume your training data is correct — the Nix/devenv ecosystem evolves quickly
2. **Do** use `search_options` on the MCP server for the authoritative answer
3. **Do** check the [official options reference](https://github.com/cachix/devenv/blob/main/docs/src/reference/options.md)
4. **Do** try the minimal reproduction approach: strip the config down to just the failing option
5. **Do** read the full Nix trace with `devenv shell -vvv` — the innermost error is often the root cause
