...
inputs = {
  ...
  # Only works when the intermediate flake declares nixpkgs as an input.
  # If it doesn't, follow the root nixpkgs directly:
  another-overlay.url = "github:owner/another-overlay";
  another-overlay.inputs.nixpkgs.follows = "nixpkgs";  # ← root nixpkgs
};
```
...
Note: `config.allowUnfree` and `config.allowUnfreePredicate` in flake.nix don't work with `nix develop` — use nixpkgs-unfree or user config instead.
...
`config.allowUnfree` and `config.allowUnfreePredicate` in flake.nix don't propagate to `nix develop`. Use:
